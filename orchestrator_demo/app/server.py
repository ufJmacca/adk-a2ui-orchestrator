"""Local HTTP transport for the orchestrator demo.

The server intentionally uses the Python standard library. The demo only needs
small harness-visible JSON endpoints and static renderer assets, so adding an
ASGI stack would increase the dependency surface without improving the slice.
"""

from __future__ import annotations

import asyncio
from collections.abc import Mapping, Sequence
from copy import deepcopy
from dataclasses import dataclass, field, is_dataclass
import dataclasses
from http import HTTPStatus
from http.server import BaseHTTPRequestHandler, ThreadingHTTPServer
import json
from pathlib import Path
import threading
from typing import Any
from urllib.parse import unquote, urlparse

from orchestrator_demo.a2a_support.transport import A2APart
from orchestrator_demo.a2ui_support.event_parser import (
    PlanUserActionParseError,
    StructuredUserActionRequiredError,
    UnsupportedUserActionError,
)
from pydantic import BaseModel
from orchestrator_demo.contracts import SpecialistResponse, StatusEvent
from orchestrator_demo.orchestrator.approval_state import ApprovalStateError
from orchestrator_demo.orchestrator.graph_runtime import GraphRuntimeError
from orchestrator_demo.orchestrator.service import (
    OrchestratorRequestResult,
    OrchestratorService,
    OrchestratorUserActionResult,
)


STATIC_ROOT = Path(__file__).resolve().parent / "static"
BLOCKED_DATA_PATH_SEGMENTS = {"__proto__", "constructor", "prototype"}


class LocalOrchestratorApp:
    """In-process application state behind the local HTTP endpoints."""

    def __init__(self, service: OrchestratorService | None = None) -> None:
        self._service = service or OrchestratorService()
        self._counter = 0
        self._counter_lock = threading.Lock()
        self._latest_a2ui_parts: list[dict[str, Any]] = []
        self._a2ui_replay_cache = _A2uiReplayCache()
        self._latest_artifacts: dict[str, Any] = {}
        self._status_events: list[dict[str, Any]] = []
        self._state_lock = threading.RLock()

    def documented_endpoints(self) -> dict[str, dict[str, str]]:
        """Return the local transport contract exposed to the harness."""

        return {
            "submit_request": {
                "method": "POST",
                "path": "/api/request",
                "requestContentType": "application/json",
                "responseContentType": "application/json",
            },
            "submit_user_action": {
                "method": "POST",
                "path": "/api/user-action",
                "requestContentType": "application/json",
                "responseContentType": "application/json",
            },
            "fetch_status_list": {
                "method": "GET",
                "path": "/api/status",
                "responseContentType": "application/json",
            },
            "fetch_status_stream": {
                "method": "GET",
                "path": "/api/status/stream",
                "responseContentType": "text/event-stream",
            },
            "fetch_latest_artifacts": {
                "method": "GET",
                "path": "/api/artifacts",
                "responseContentType": "application/json",
            },
            "renderer": {
                "method": "GET",
                "path": "/",
                "responseContentType": "text/html",
            },
        }

    def submit_request(self, payload: Mapping[str, Any]) -> dict[str, Any]:
        """Submit a natural-language user request to the orchestrator."""

        with self._state_lock:
            user_input = _required_text(payload, "input", "userInput", "user_input")
            task_id, context_id = self._next_transport_ids()
            previous_surface_owners = self._service.clear_renderer_surfaces()
            try:
                result = asyncio.run(self._service.handle_user_request(user_input))
            except Exception:
                self._service.restore_renderer_surfaces(previous_surface_owners)
                raise
            response = self._request_response_payload(
                result,
                task_id=task_id,
                context_id=context_id,
            )
            self._replace_result_a2ui(response["a2uiParts"])
            self._latest_artifacts = response["artifacts"]
            self._record_request_status(result, task_id=task_id)
            return response

    def submit_user_action(self, payload: Mapping[str, Any]) -> dict[str, Any]:
        """Submit a structured A2UI ``userAction`` event."""

        with self._state_lock:
            try:
                result = asyncio.run(self._service.handle_user_action(payload))
            except GraphRuntimeError as exc:
                self._record_graph_runtime_error_status(exc)
                raise
            response = self._user_action_response_payload(result)
            if response["a2uiParts"]:
                self._merge_result_a2ui(response["a2uiParts"])
            if response["artifacts"]:
                self._latest_artifacts = response["artifacts"]
            self._record_user_action_status(result)
            return response

    def status_list(self) -> dict[str, Any]:
        """Return accumulated app and graph status events."""

        with self._state_lock:
            return {"statusEvents": list(self._status_events)}

    def status_stream(self) -> str:
        """Return a small Server-Sent Events snapshot of current statuses."""

        with self._state_lock:
            lines: list[str] = []
            for event in self._status_events:
                lines.append("event: status")
                lines.append(f"data: {json.dumps(event, separators=(',', ':'))}")
                lines.append("")
            return "\n".join(lines) + ("\n" if lines else "")

    def latest_artifacts(self) -> dict[str, Any]:
        """Return the latest final artifacts and renderer A2UI parts."""

        with self._state_lock:
            return {
                "artifacts": _jsonable(self._latest_artifacts),
                "a2uiParts": list(self._latest_a2ui_parts),
            }

    def _next_transport_ids(self) -> tuple[str, str]:
        with self._counter_lock:
            self._counter += 1
            next_id = self._counter
        return f"task_local_{next_id}", f"ctx_local_{next_id}"

    def _request_response_payload(
        self,
        result: OrchestratorRequestResult,
        *,
        task_id: str,
        context_id: str,
    ) -> dict[str, Any]:
        return {
            "taskId": task_id,
            "contextId": context_id,
            "path": result.path,
            "decision": _jsonable(result.decision),
            "approvalPlan": _jsonable(result.approval_plan),
            "approvalResult": _approval_result_payload(result.approval_result),
            "specialistResponses": _jsonable(result.specialist_responses),
            "a2uiParts": _data_parts_payload(result.a2ui_parts),
            "statusEvents": _status_events_payload(result.status_events),
            "artifacts": _jsonable(result.final_artifacts),
        }

    def _user_action_response_payload(
        self,
        result: OrchestratorUserActionResult,
    ) -> dict[str, Any]:
        status = _public_user_action_status(result.status)
        status_events = _user_action_status_events_payload(result)
        return {
            "status": status,
            "approvalResult": _approval_result_payload(result.approval_result),
            "surfaceRouteResult": _surface_route_payload(result),
            "specialistResponses": _jsonable(result.specialist_responses),
            "a2uiParts": _data_parts_payload(result.a2ui_parts),
            "statusEvents": status_events,
            "artifacts": _jsonable(result.final_artifacts),
        }

    def _record_request_status(
        self,
        result: OrchestratorRequestResult,
        *,
        task_id: str,
    ) -> None:
        if result.path == "plan_required" and result.approval_plan is not None:
            self._status_events.append(
                {
                    "status": "approval_required",
                    "message": "Plan approval is pending.",
                    "taskId": task_id,
                    "planId": result.approval_plan.plan_id,
                }
            )
            return

        self._status_events.append(
            {
                "status": result.path,
                "message": "Request handled.",
                "taskId": task_id,
                "planId": None,
            }
        )

    def _record_user_action_status(
        self,
        result: OrchestratorUserActionResult,
    ) -> None:
        status_events = _user_action_status_events_payload(result)
        if status_events:
            self._status_events.extend(status_events)
            return

        plan_id = (
            result.approval_result.plan_id
            if result.approval_result is not None
            else None
        )
        status = _public_user_action_status(result.status)
        message = _user_action_status_message(status)
        self._status_events.append(
            {
                "status": status,
                "message": message,
                "taskId": None,
                "planId": plan_id,
            }
        )

    def _record_graph_runtime_error_status(self, exc: GraphRuntimeError) -> None:
        self._status_events.extend(_public_error_status_events_payload(exc))

    def _replace_result_a2ui(self, a2ui_parts: list[dict[str, Any]]) -> None:
        self._a2ui_replay_cache.clear()
        if not a2ui_parts:
            self._service.clear_renderer_surfaces()
            self._latest_a2ui_parts = []
            return
        self._merge_result_a2ui(a2ui_parts)

    def _merge_result_a2ui(self, a2ui_parts: list[dict[str, Any]]) -> None:
        self._a2ui_replay_cache.apply(a2ui_parts)
        self._latest_a2ui_parts = self._a2ui_replay_cache.parts()


@dataclass
class _A2uiSurfaceSnapshot:
    create_part: dict[str, Any] | None = None
    data_model_message: dict[str, Any] | None = None
    data_model: Any = field(default_factory=dict)
    update_components_message: dict[str, Any] | None = None
    components_by_id: dict[str, dict[str, Any]] = field(default_factory=dict)
    component_order: list[str] = field(default_factory=list)


class _A2uiReplayCache:
    """Maintain page-load replay state for incremental renderer updates."""

    def __init__(self) -> None:
        self._surface_order: list[str] = []
        self._surfaces: dict[str, _A2uiSurfaceSnapshot] = {}

    def clear(self) -> None:
        self._surface_order.clear()
        self._surfaces.clear()

    def apply(self, parts: list[dict[str, Any]]) -> None:
        for part in parts:
            payload = _part_payload(part)
            if payload is None:
                continue
            self._apply_payload(part, payload)

    def parts(self) -> list[dict[str, Any]]:
        replay_parts: list[dict[str, Any]] = []
        for surface_id in self._surface_order:
            snapshot = self._surfaces.get(surface_id)
            if snapshot is None:
                continue
            if snapshot.create_part is not None:
                replay_parts.append(deepcopy(snapshot.create_part))
            if snapshot.data_model_message is not None:
                replay_parts.append(_wrapped_part(snapshot.data_model_message))
            if snapshot.update_components_message is not None:
                update_payload = deepcopy(snapshot.update_components_message)
                update = update_payload.get("updateComponents")
                if isinstance(update, dict):
                    update["components"] = [
                        deepcopy(snapshot.components_by_id[component_id])
                        for component_id in snapshot.component_order
                        if component_id in snapshot.components_by_id
                    ]
                replay_parts.append(_wrapped_part(update_payload))
        return replay_parts

    def _apply_payload(
        self,
        part: Mapping[str, Any],
        payload: Mapping[str, Any],
    ) -> None:
        create_surface = payload.get("createSurface")
        if isinstance(create_surface, Mapping):
            surface_id = create_surface.get("surfaceId")
            if isinstance(surface_id, str):
                snapshot = self._snapshot_for(surface_id)
                snapshot.create_part = deepcopy(dict(part))
                return

        update_data_model = payload.get("updateDataModel")
        if isinstance(update_data_model, Mapping):
            surface_id = update_data_model.get("surfaceId")
            if isinstance(surface_id, str):
                snapshot = self._snapshot_for(surface_id)
                snapshot.data_model = _applied_data_model_update(
                    snapshot.data_model,
                    update_data_model,
                )
                snapshot.data_model_message = _data_model_payload_template(
                    payload,
                    snapshot.data_model,
                )
                return

        update_components = payload.get("updateComponents")
        if isinstance(update_components, Mapping):
            surface_id = update_components.get("surfaceId")
            if isinstance(surface_id, str):
                snapshot = self._snapshot_for(surface_id)
                if _is_full_replacement(update_components):
                    snapshot.components_by_id.clear()
                    snapshot.component_order.clear()
                snapshot.update_components_message = _components_payload_template(
                    payload,
                )
                for component in update_components.get("components", []):
                    if not isinstance(component, Mapping):
                        continue
                    component_id = component.get("id")
                    if not isinstance(component_id, str):
                        continue
                    if component_id not in snapshot.components_by_id:
                        snapshot.component_order.append(component_id)
                    snapshot.components_by_id[component_id] = deepcopy(
                        dict(component)
                    )
                return

        delete_surface = payload.get("deleteSurface")
        if isinstance(delete_surface, Mapping):
            surface_id = delete_surface.get("surfaceId")
            if isinstance(surface_id, str):
                self._surfaces.pop(surface_id, None)
                self._surface_order = [
                    existing_id
                    for existing_id in self._surface_order
                    if existing_id != surface_id
                ]

    def _snapshot_for(self, surface_id: str) -> _A2uiSurfaceSnapshot:
        snapshot = self._surfaces.get(surface_id)
        if snapshot is None:
            snapshot = _A2uiSurfaceSnapshot()
            self._surfaces[surface_id] = snapshot
            self._surface_order.append(surface_id)
        return snapshot


class LocalHttpServer:
    """Lifecycle wrapper for tests, harnesses, and the module entrypoint."""

    def __init__(self, httpd: ThreadingHTTPServer) -> None:
        self._httpd = httpd
        self._thread: threading.Thread | None = None
        self._serve_started = threading.Event()

    @property
    def base_url(self) -> str:
        host, port = self._httpd.server_address[:2]
        host_text = host.decode("ascii") if isinstance(host, bytes) else host
        return f"http://{host_text}:{port}"

    def start_in_thread(self) -> None:
        if self._thread is not None:
            return
        self._thread = threading.Thread(
            target=self._serve_forever,
            name="orchestrator-demo-http",
            daemon=True,
        )
        self._thread.start()

    def serve_forever(self) -> None:
        self._serve_forever()

    def stop(self) -> None:
        if self._thread is not None and not self._serve_started.is_set():
            self._serve_started.wait(timeout=1)
        if self._serve_started.is_set():
            self._httpd.shutdown()
        self._httpd.server_close()
        if self._thread is not None:
            self._thread.join(timeout=10)
            self._thread = None

    def _serve_forever(self) -> None:
        self._serve_started.set()
        try:
            self._httpd.serve_forever()
        finally:
            self._serve_started.clear()


class _LocalHttpServer(ThreadingHTTPServer):
    daemon_threads = True
    allow_reuse_address = True

    def __init__(
        self,
        server_address: tuple[str, int],
        app: LocalOrchestratorApp,
    ) -> None:
        self.app = app
        super().__init__(server_address, _RequestHandler)


class _RequestHandler(BaseHTTPRequestHandler):
    server: _LocalHttpServer

    def log_message(self, _format: str, *_args: Any) -> None:
        """Suppress default request logging to avoid noisy harness output."""

    def do_GET(self) -> None:  # noqa: N802
        path = urlparse(self.path).path
        if path == "/api":
            self._write_json({"endpoints": self.server.app.documented_endpoints()})
            return
        if path == "/api/status":
            self._write_json(self.server.app.status_list())
            return
        if path == "/api/status/stream":
            self._write_text(
                self.server.app.status_stream(),
                content_type="text/event-stream; charset=utf-8",
            )
            return
        if path == "/api/artifacts":
            self._write_json(self.server.app.latest_artifacts())
            return
        if path == "/" or path == "/index.html":
            self._write_static_file(STATIC_ROOT / "index.html")
            return
        if path.startswith("/static/"):
            self._write_static_file(_static_path(path))
            return
        self._write_json({"error": "not found"}, status=HTTPStatus.NOT_FOUND)

    def do_POST(self) -> None:  # noqa: N802
        try:
            payload = self._read_json_body()
            path = urlparse(self.path).path
            if path == "/api/request":
                _required_text(payload, "input", "userInput", "user_input")
                try:
                    self._write_json(self.server.app.submit_request(payload))
                except Exception as exc:
                    self._write_json(
                        _runtime_error_payload(exc),
                        status=HTTPStatus.INTERNAL_SERVER_ERROR,
                    )
                return
            if path == "/api/user-action":
                try:
                    self._write_json(self.server.app.submit_user_action(payload))
                except (
                    ApprovalStateError,
                    PlanUserActionParseError,
                    StructuredUserActionRequiredError,
                    UnsupportedUserActionError,
                ) as exc:
                    self._write_json(
                        _user_action_error_payload(exc),
                        status=HTTPStatus.CONFLICT
                        if isinstance(exc, ApprovalStateError)
                        else HTTPStatus.BAD_REQUEST,
                    )
                except Exception as exc:
                    self._write_json(
                        _runtime_error_payload(exc),
                        status=HTTPStatus.INTERNAL_SERVER_ERROR,
                    )
                return
            self._write_json({"error": "not found"}, status=HTTPStatus.NOT_FOUND)
        except ValueError as exc:
            self._write_json({"error": str(exc)}, status=HTTPStatus.BAD_REQUEST)

    def _read_json_body(self) -> dict[str, Any]:
        content_length = int(self.headers.get("content-length", "0"))
        raw_body = self.rfile.read(content_length)
        if not raw_body:
            return {}
        try:
            decoded = json.loads(raw_body.decode("utf-8"))
        except (UnicodeDecodeError, json.JSONDecodeError) as exc:
            raise ValueError("request body must be valid JSON") from exc
        if not isinstance(decoded, dict):
            raise ValueError("request body must be a JSON object")
        return decoded

    def _write_static_file(self, path: Path) -> None:
        try:
            resolved_path = path.resolve(strict=True)
        except FileNotFoundError:
            self._write_json({"error": "not found"}, status=HTTPStatus.NOT_FOUND)
            return
        if STATIC_ROOT not in resolved_path.parents and resolved_path != STATIC_ROOT:
            self._write_json({"error": "not found"}, status=HTTPStatus.NOT_FOUND)
            return
        if not resolved_path.is_file():
            self._write_json({"error": "not found"}, status=HTTPStatus.NOT_FOUND)
            return
        content_type = _content_type_for(resolved_path)
        body = resolved_path.read_bytes()
        self.send_response(HTTPStatus.OK)
        self.send_header("content-type", content_type)
        self.send_header("content-length", str(len(body)))
        self.end_headers()
        self.wfile.write(body)

    def _write_json(
        self,
        payload: Mapping[str, Any],
        *,
        status: HTTPStatus = HTTPStatus.OK,
    ) -> None:
        body = json.dumps(_jsonable(payload), separators=(",", ":")).encode("utf-8")
        self.send_response(status)
        self.send_header("content-type", "application/json; charset=utf-8")
        self.send_header("content-length", str(len(body)))
        self.end_headers()
        self.wfile.write(body)

    def _write_text(
        self,
        text: str,
        *,
        content_type: str,
        status: HTTPStatus = HTTPStatus.OK,
    ) -> None:
        body = text.encode("utf-8")
        self.send_response(status)
        self.send_header("content-type", content_type)
        self.send_header("content-length", str(len(body)))
        self.end_headers()
        self.wfile.write(body)


def create_server(
    *,
    host: str = "127.0.0.1",
    port: int = 8000,
    app: LocalOrchestratorApp | None = None,
) -> LocalHttpServer:
    """Create a local HTTP server for the orchestrator app."""

    httpd = _LocalHttpServer((host, port), app or LocalOrchestratorApp())
    return LocalHttpServer(httpd)


def _required_text(payload: Mapping[str, Any], *keys: str) -> str:
    for key in keys:
        value = payload.get(key)
        if isinstance(value, str) and value.strip():
            return value
    joined = ", ".join(keys)
    raise ValueError(f"request requires one non-empty text field: {joined}")


def _static_path(path: str) -> Path:
    relative = unquote(path.removeprefix("/static/"))
    return STATIC_ROOT / relative


def _content_type_for(path: Path) -> str:
    if path.suffix == ".html":
        return "text/html; charset=utf-8"
    if path.suffix == ".js":
        return "text/javascript; charset=utf-8"
    if path.suffix == ".css":
        return "text/css; charset=utf-8"
    return "application/octet-stream"


def _part_payload(part: Mapping[str, Any]) -> Mapping[str, Any] | None:
    data = part.get("data")
    if isinstance(data, Mapping):
        return data
    if any(
        message_type in part
        for message_type in (
            "createSurface",
            "updateComponents",
            "updateDataModel",
            "deleteSurface",
        )
    ):
        return part
    return None


def _wrapped_part(payload: Mapping[str, Any]) -> dict[str, Any]:
    return {
        "type": "data",
        "data": deepcopy(dict(payload)),
        "metadata": {"mimeType": "application/json+a2ui"},
    }


def _components_payload_template(payload: Mapping[str, Any]) -> dict[str, Any]:
    template = deepcopy(dict(payload))
    update_components = template.get("updateComponents")
    if isinstance(update_components, dict):
        update_components["components"] = []
        update_components.pop("replace", None)
        update_components.pop("fullReplacement", None)
        if update_components.get("mode") == "replace":
            update_components.pop("mode", None)
    return template


def _data_model_payload_template(
    payload: Mapping[str, Any],
    data_model: Any,
) -> dict[str, Any]:
    template = deepcopy(dict(payload))
    update_data_model = template.get("updateDataModel")
    if isinstance(update_data_model, dict):
        surface_id = update_data_model.get("surfaceId")
        template["updateDataModel"] = {
            "surfaceId": surface_id,
            "path": "/",
            "value": deepcopy(data_model),
        }
    return template


def _applied_data_model_update(
    current_data_model: Any,
    update_data_model: Mapping[str, Any],
) -> Any:
    has_path = "path" in update_data_model
    has_value = "value" in update_data_model
    if not has_path and not has_value:
        data = update_data_model.get("data")
        return deepcopy(data) if isinstance(data, Mapping) else {}

    parts = _safe_data_path_parts(update_data_model.get("path", "/"))
    if parts is None:
        return deepcopy(current_data_model)
    if not parts:
        return deepcopy(update_data_model["value"]) if has_value else {}

    next_data_model = deepcopy(current_data_model)
    if not isinstance(next_data_model, dict | list):
        next_data_model = {}
    if has_value:
        _set_data_model_path(next_data_model, parts, update_data_model["value"])
    else:
        _delete_data_model_path(next_data_model, parts)
    return next_data_model


def _set_data_model_path(data_model: Any, parts: list[str], value: Any) -> None:
    current = data_model
    for part in parts[:-1]:
        if isinstance(current, list) and part.isdecimal():
            index = int(part)
            if index >= len(current):
                current.extend({} for _ in range(index - len(current) + 1))
            if not isinstance(current[index], dict | list):
                current[index] = {}
            current = current[index]
            continue
        if not isinstance(current, dict):
            return
        child = current.get(part)
        if not isinstance(child, dict | list):
            child = {}
            current[part] = child
        current = child

    last_part = parts[-1]
    if isinstance(current, list) and last_part.isdecimal():
        index = int(last_part)
        if index >= len(current):
            current.extend(None for _ in range(index - len(current) + 1))
        current[index] = deepcopy(value)
        return
    if isinstance(current, dict):
        current[last_part] = deepcopy(value)


def _delete_data_model_path(data_model: Any, parts: list[str]) -> None:
    current = data_model
    for part in parts[:-1]:
        if isinstance(current, list) and part.isdecimal():
            index = int(part)
            if index >= len(current):
                return
            current = current[index]
            continue
        if not isinstance(current, Mapping):
            return
        current = current.get(part)

    last_part = parts[-1]
    if isinstance(current, list) and last_part.isdecimal():
        index = int(last_part)
        if index < len(current):
            current.pop(index)
        return
    if isinstance(current, dict):
        current.pop(last_part, None)


def _safe_data_path_parts(path: Any) -> list[str] | None:
    if not isinstance(path, str):
        return None
    parts = [
        _decode_json_pointer_part(part)
        for part in path.split("/")
        if part
    ]
    if any(part in BLOCKED_DATA_PATH_SEGMENTS for part in parts):
        return None
    return parts


def _decode_json_pointer_part(part: str) -> str:
    return part.replace("~1", "/").replace("~0", "~")


def _is_full_replacement(message: Mapping[str, Any]) -> bool:
    return (
        message.get("replace") is True
        or message.get("fullReplacement") is True
        or message.get("mode") == "replace"
    )


def _data_parts_payload(parts: Sequence[A2APart]) -> list[dict[str, Any]]:
    return [part.model_dump(by_alias=True, mode="json") for part in parts]


def _status_events_payload(events: Sequence[StatusEvent]) -> list[dict[str, Any]]:
    return [event.model_dump(mode="json") for event in events]


def _user_action_status_events_payload(
    result: OrchestratorUserActionResult,
) -> list[dict[str, Any]]:
    if result.status_events:
        return _status_events_payload(result.status_events)
    if _public_user_action_status(result.status) == "error":
        return [_user_action_error_status_event(result)]
    return []


def _user_action_error_status_event(
    result: OrchestratorUserActionResult,
) -> dict[str, Any]:
    plan_id = (
        result.approval_result.plan_id
        if result.approval_result is not None
        else None
    )
    route_error = (
        result.surface_route_result.error
        if result.surface_route_result is not None
        else None
    )
    message = _user_action_status_message("error")
    details: dict[str, Any] = {}
    if isinstance(route_error, Mapping):
        route_message = route_error.get("message")
        if isinstance(route_message, str) and route_message:
            message = route_message
        details = {
            str(key): _jsonable(value)
            for key, value in route_error.items()
            if key != "message"
        }

    event: dict[str, Any] = {
        "status": "error",
        "message": message,
        "taskId": None,
        "planId": plan_id,
    }
    if details:
        event["details"] = details
    return event


def _public_error_status_events_payload(exc: Exception) -> list[dict[str, Any]]:
    events = _status_events_payload(getattr(exc, "status_events", ()))
    if not isinstance(exc, GraphRuntimeError):
        return events

    public_events: list[dict[str, Any]] = []
    for event in events:
        if event.get("status") != "step_failed":
            public_events.append(event)
            continue

        public_event = dict(event)
        public_event["message"] = (
            "Approved plan step failed during execution. The draft remains "
            "retryable."
        )
        details = public_event.get("details")
        if isinstance(details, Mapping):
            public_event["details"] = {
                str(key): _jsonable(value)
                for key, value in details.items()
                if key != "developerMessage"
            }
        public_events.append(public_event)
    return public_events


def _approval_result_payload(result: Any) -> dict[str, Any] | None:
    if result is None:
        return None
    return {
        "status": result.status,
        "planId": result.plan_id,
        "planVersion": result.plan_version,
        "approvedPlan": _jsonable(result.approved_plan),
        "reason": result.rejection_reason,
        "failureReason": result.failure_reason,
        "graphCreated": result.graph_created,
        "specialistsCalled": result.specialists_called,
    }


def _surface_route_payload(
    result: OrchestratorUserActionResult,
) -> dict[str, Any] | None:
    route_result = result.surface_route_result
    if route_result is None:
        return None
    owner = route_result.owner
    return {
        "status": _public_user_action_status(route_result.status),
        "owner": None
        if owner is None
        else {
            "surfaceId": owner.surface_id,
            "ownerType": owner.owner_type,
            "ownerId": owner.owner_id,
            "planId": owner.plan_id,
        },
        "error": _jsonable(route_result.error),
    }


def _runtime_error_payload(exc: Exception) -> dict[str, Any]:
    code = (
        "graph_execution_failed"
        if isinstance(exc, GraphRuntimeError)
        else "runtime_error"
    )
    return {
        "status": "error",
        "error": {
            "code": code,
            "message": _public_runtime_error_message(code),
            "retryable": True,
        },
        "statusEvents": _public_error_status_events_payload(exc),
    }


def _public_runtime_error_message(code: str) -> str:
    if code == "graph_execution_failed":
        return "Graph execution failed. The draft approval remains retryable."
    return "Request failed while processing. Retry after checking local service logs."


def _user_action_error_payload(exc: Exception) -> dict[str, Any]:
    code = (
        "approval_state_conflict"
        if isinstance(exc, ApprovalStateError)
        else "invalid_user_action"
    )
    return {
        "status": "error",
        "error": {
            "code": code,
            "message": _public_user_action_error_message(code),
            "retryable": False,
        },
        "statusEvents": [],
    }


def _public_user_action_error_message(code: str) -> str:
    if code == "approval_state_conflict":
        return (
            "User action cannot be applied to the current approval state. "
            "Refresh the plan and submit a current structured userAction."
        )
    return "User action payload is invalid for the requested surface."


def _user_action_status_message(status: str) -> str:
    return {
        "draft_updated": "Plan draft updated.",
        "rejected": "Plan rejected.",
        "approved": "Plan approved.",
        "failed": "Plan approval failed.",
        "routed": "A2UI event routed to surface owner.",
        "error": "User action failed.",
    }.get(status, "User action handled.")


def _public_user_action_status(status: str) -> str:
    if status == "forwarded":
        return "routed"
    return status


def _jsonable(value: Any) -> Any:
    if value is None or isinstance(value, str | int | float | bool):
        return value
    if isinstance(value, BaseModel):
        return value.model_dump(mode="json")
    if isinstance(value, SpecialistResponse):
        return value.model_dump(mode="json")
    if is_dataclass(value) and not isinstance(value, type):
        return _jsonable(dataclasses.asdict(value))
    if isinstance(value, Mapping):
        return {str(key): _jsonable(child) for key, child in value.items()}
    if isinstance(value, tuple | list):
        return [_jsonable(child) for child in value]
    return str(value)


__all__ = [
    "LocalHttpServer",
    "LocalOrchestratorApp",
    "create_server",
]
