...
function renderComponent(surfaceId, component) {
  const renderer = COMPONENT_RENDERERS[component.component];
  if (!renderer) {
...
function renderChildren(surfaceId, childIds) {
  const surface = surfaceFor(surfaceId);
  if (!surface) {
    return [];
  }
...
function renderCard(surfaceId, component) {
  ...
  if (component.child) {
    element.replaceChildren(...renderChildren(surfaceId, [component.child]));
  }
  return element;
}
...
function renderUnsupportedComponent(component) {
  ...
  element.textContent = `Unsupported component: ${component.component || 'unknown'}`;
  return element;
}
